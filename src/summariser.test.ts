...
const lecture2_keywords = new Set<String>(['memory', 'factors', 'influence', 'overlearning', 'study', 'recall', 'perfectly', 'topic', 'continue', 'easier', 'organization', 'material', 'strategies', 'clustering', 'recognize', 'probability', 'groups', 'pneumonic', 'acrostics', 'phrases', 'cue', 'information', 'cranial', 'nerves', 'anatomy', 'physiology'])

test('accurate summary of short transcript', async () => {
	const text = lecture1.getText();
    const summary = await summarise(text, -1, false, 1);
...
test('accurate brief summary of short transcript', async () => {
	const text = lecture1.getText();
    const summary = await summarise(text, -1, false, 2);
    const keyWords = lecture1_keywords;
...
test('accurate extensive summary of short transcript', async () => {
	const text = lecture1.getText();
    const summary = await summarise(text, -1, false, 3);
    const keyWords = lecture1_keywords;
