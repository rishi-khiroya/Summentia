...
export const load: PageServerLoad = async (event) => {
    ...
    const fullSummary = () => {
        ...
        if(project.hasSlides){
            summaryData.forEach((item) => item.summaries.forEach((summary_object) => {summary = summary + " " + summary_object}))
        } else {
...
function printFlashCards(flashCards: Flashcard[]){
    let print = "";
    flashCards.forEach((card) => {print +=  ("asfront:" + card.front + " ssback: " +card.back)})
    return print;
}